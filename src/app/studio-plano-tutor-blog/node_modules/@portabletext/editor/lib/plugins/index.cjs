...
const emphasisListener = ({
  ...
  behavior: behaviors_index.defineBehavior({
    ...
    guard: ({
      ...
      const anchor = selectionPoint.spanSelectionPointToBlockOffset({
        context: snapshot.context,
        selectionPoint: event.at.anchor
      }), focus = selectionPoint.spanSelectionPointToBlockOffset({
        context: snapshot.context,
  ...
  const automaticBlockquoteOnSpace = behaviors_index.defineBehavior({
    ...
    guard: ({
      ...
      if (!selectionCollapsed || !focusTextBlock || !focusSpan)
        return !1;
      const previousInlineObject = selector_isSelectionExpanded.getPreviousInlineObject(snapshot), blockOffset = selectionPoint.spanSelectionPointToBlockOffset({
        context: snapshot.context,
        selectionPoint: {
          path: [{
      ...
      if (previousInlineObject || !blockOffset)
        return !1;
      const blockText = selectionPoint.getTextBlockText(focusTextBlock.node), caretAtTheEndOfQuote = blockOffset.offset === 1, looksLikeMarkdownQuote = /^>/.test(blockText), blockquoteStyle = config.blockquoteStyle?.(snapshot.context);
    ...
    actions: [(_, {
      ...
      hrObject,
      focusBlock
    }) => selectionPoint.isTextBlock(snapshot.context, focusBlock.node) ? [behaviors_index.execute({
      type: "insert.block",
      block: {
    ...
    guard: ({
      ...
      if (!selectionCollapsed || !focusTextBlock || !focusSpan)
        return !1;
      const blockOffset = selectionPoint.spanSelectionPointToBlockOffset({
        context: snapshot.context,
        selectionPoint: {
          path: [{
            _key: focusTextBlock.node._key
      ...
      if (!blockOffset)
        return !1;
      const previousInlineObject = selector_isSelectionExpanded.getPreviousInlineObject(snapshot), blockText = selectionPoint.getTextBlockText(focusTextBlock.node), markdownHeadingSearch = /^#+/.exec(blockText), level = markdownHeadingSearch ? markdownHeadingSearch[0].length : void 0, caretAtTheEndOfHeading = blockOffset.offset === level;
    ...
    guard: ({
      ...
      if (previousInlineObject || !blockOffset)
        return !1;
      const blockText = selectionPoint.getTextBlockText(focusTextBlock.node), defaultStyle = config.defaultStyle?.(snapshot.context), looksLikeUnorderedList = /^(-|\*)/.test(blockText), unorderedListStyle = config.unorderedListStyle?.(snapshot.context), caretAtTheEndOfUnorderedList = blockOffset.offset === 1;
